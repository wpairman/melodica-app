        ...
        request.onsuccess = () => {
          ...
          getAllRequest.onsuccess = () => {
            ...
            if (indexedDBMoods.length > 0) {
              ...
              let moodHistory: any[] = []
              if (storedHistory) {
                try { moodHistory = JSON.parse(storedHistory) } catch (e) { moodHistory = [] }
              }
              indexedDBMoods.forEach((entry: any) => {
                const newEntry = { mood: entry.mood, timestamp: new Date(entry.timestamp), notes: entry.notes || "" }
                const exists = moodHistory.some((existing: any) => {
                  return new Date(existing.timestamp).toISOString() === newEntry.timestamp.toISOString() && existing.mood === newEntry.mood
                })
                if (!exists) moodHistory.push(newEntry)
              })
              localStorage.setItem("moodHistory", JSON.stringify(moodHistory))
              const clearTransaction = db.transaction(['moods'], 'readwrite')
              clearTransaction.objectStore('moods').clear()
            }
          }
        }
      } catch (error) {
    ...
    if ('serviceWorker' in navigator) {
      const messageHandler = (event: MessageEvent) => {
        if (event.data && event.data.type === 'QUICK_MOOD_LOG') {
          ...
          let moodHistory: any[] = []
          if (storedHistory) {
            try { moodHistory = JSON.parse(storedHistory) } catch (e) {}
          }
          const newEntry = { mood: moodEntry.mood, timestamp: new Date(moodEntry.timestamp), notes: moodEntry.notes || "" }
          const exists = moodHistory.some((entry: any) => {
            return new Date(entry.timestamp).toISOString() === newEntry.timestamp.toISOString() && entry.mood === newEntry.mood
          })
          if (!exists) {
            moodHistory.push(newEntry)
            localStorage.setItem("moodHistory", JSON.stringify(moodHistory))
            setMoodHistory(moodHistory)
            toast({ title: "Mood logged! 💚", description: `Your ${moodEntry.mood}-star mood has been saved.` })
          }
        }
      }
                ...
                <div className="px-4 py-2">
                  ...
                  <div className="space-y-1">
                    ...
                    <Link href="/dashboard/mood-history">
                      <Button variant="ghost" className="w-full justify-start text-white hover:bg-gray-800">
                        <Activity className="mr-2 h-4 w-4" /> Mood History
                      </Button>
                    </Link>
                    <Link href="/analytics">
                      <Button variant="ghost" className="w-full justify-start text-white hover:bg-gray-800">
                        <TrendingUp className="mr-2 h-4 w-4" /> Mood Analytics
                      </Button>
                    </Link>
                    <Link href="/weather-mood">
                      <Button variant="ghost" className="w-full justify-start text-white hover:bg-gray-800">
                        <Cloud className="mr-2 h-4 w-4" /> Weather & Mood
                      </Button>
                    </Link>
                    ...
                    <Link href="/music-preferences">
                      <Button variant="ghost" className="w-full justify-start text-white hover:bg-gray-800">
                        <Music className="mr-2 h-4 w-4" /> Your Artist's Songs
                      </Button>
                    </Link>
                    <Link href="/dashboard/playlists">
                      <Button variant="ghost" className="w-full justify-start text-white hover:bg-gray-800">
                        <ListMusic className="mr-2 h-4 w-4" /> Playlists
                      </Button>
                    </Link>
                    ...
                    <Link href="/dashboard/jamaica-wellness">
                      <Button variant="ghost" className="w-full justify-start text-white hover:bg-gray-800">
                        <Sparkles className="mr-2 h-4 w-4" /> Jamaica Wellness Toolkit
                      </Button>
                    </Link>
                    ...
                    {userData.gender === "female" && (
                      <Link href="/period-tracker">
                        <Button variant="ghost" className="w-full justify-start text-white hover:bg-gray-800">
                          <Droplets className="mr-2 h-4 w-4" /> Period Tracking
                        </Button>
                      </Link>
                    )}
                  </div>
              ...
              <main className="container mx-auto p-4 md:p-6">
                <div className="flex flex-col gap-6">
                  ...
                  <Tabs defaultValue="mood" className="w-full">
                    {/* Horizontally scrollable tabs — no wrapping on mobile */}
